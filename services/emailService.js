...
// Send emergency email to contacts using Brevo
const sendEmergencyEmail = async (user, emergencyAlert, location = null) => {
  try {
    ...
    if (!brevoClient) {
      console.log('📧 Brevo service not configured - skipping email notifications');
    ...
    console.log(`📧 Brevo API: ${sent} sent, ${failed} failed out of ${emergencyContacts.length} contacts`);
    
    results.forEach((result, index) => {
      const contact = emergencyContacts[index];
      if (result.status === 'fulfilled') {
        console.log(`✅ Email sent via Brevo to: ${contact.email} (${contact.name})`);
      } else {
        console.error(`❌ Failed to send email via Brevo to: ${contact.email} (${contact.name})`, result.reason);
      }
    });
    ...
    console.error('❌ Error in sendEmergencyEmail (Brevo):', error);
    return { sent: 0, total: 0, failed: 0 };
  }
};
